...
\begin{equation}\label{Eq:partialsL2}
  \begin{aligned}
...
It's interesting to see that the step in taking the partial derivatives of $\mathcal{L}$ in Eq. \ref{Eq:partialsL2} is analogous to the construction/computation of dual ``conjugate'' variables, conjugate momentum, in physics.  
...
Cost functional $J$, to first input in the output values $y$.  So
\begin{equation}
\begin{aligned}
&  J: (\mathbb{R}^d \times \mathbb{R}^K)^m \to L(\mathbf{ (\Theta, b) }, \mathbb{R} ) \\ 
& J: (X,Y) \mapsto J_{(X,Y)} \equiv J 
\end{aligned}
\end{equation}
for a ``vector-valued'' regression, with the usual linear regression being the case of $K=1$.
...
\begin{equation}
\begin{aligned}
%&  J_{(X,Y)} \equiv J : \mathbf{ ( \Theta,b) } \to \mathbb{R} \\ 
& J: (\Theta,b) \to J(\Theta,b)
\end{aligned}
\end{equation}
i.e. $J \in C^{\infty}((\Theta,b))$ (hopefully $J$ is smooth or at least $C^2$ differentiable, so that a Hessian can be obtained).
...
\section{Example application: 2017 Global AI Hackathon, San Diego (June 23-25, 2017)}
...
\part{Convolution Neural Networks (CNN)}  
...
cf. http://davidstutz.de/wordpress/wp-content/uploads/2014/07/seminar.pdf
